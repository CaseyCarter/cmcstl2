...
test_iter()
{
	...
	int r1[10] = {0};
	int r2[10] = {0};
	auto p = stl2::partition_copy(Iter(std::begin(ia)),
								 Sent(std::end(ia)),
...
test_range()
{
	...
	int r1[10] = {0};
	int r2[10] = {0};
	auto p = stl2::partition_copy(::as_lvalue(stl2::subrange(Iter(std::begin(ia)),
														   Sent(std::end(ia)))),
								 output_iterator<int*>(r1), r2, is_odd());
	CHECK(p.in == Iter(std::end(ia)));
	CHECK(p.out1.base() == r1 + 4);
...
void test_proj()
{
	...
	S r1[10] = {S{0}};
	S r2[10] = {S{0}};
	auto p = stl2::partition_copy(ia, r1, r2, is_odd(), &S::i);

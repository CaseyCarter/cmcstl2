...
test_range()
{
	...
	int ja[sa] = {-1};
	count_equal::count = 0;
	std::pair<InIter, OutIter> r = stl2::unique_copy(stl2::ext::subrange(InIter(ia), Sent(ia+sa)), OutIter(ja), count_equal());
	CHECK(base(r.first) == ia + sa);
	CHECK(base(r.second) == ja + sa);
	CHECK(ja[0] == 0);
	CHECK(count_equal::count == sa-1);
	...
	int jb[sb] = {-1};
	count_equal::count = 0;
	r = stl2::unique_copy(stl2::ext::subrange(InIter(ib), Sent(ib+sb)), OutIter(jb), count_equal());
	CHECK(base(r.first) == ib + sb);
	CHECK(base(r.second) == jb + sb);
	CHECK(jb[0] == 0);
	CHECK(jb[1] == 1);
	...
	int jc[sc] = {-1};
	count_equal::count = 0;
	r = stl2::unique_copy(stl2::ext::subrange(InIter(ic), Sent(ic+sc)), OutIter(jc), count_equal());
	CHECK(base(r.first) == ic + sc);
	CHECK(base(r.second) == jc + 1);
	CHECK(jc[0] == 0);
	CHECK(count_equal::count == sc-1);
	...
	int jd[sd] = {-1};
	count_equal::count = 0;
	r = stl2::unique_copy(stl2::ext::subrange(InIter(id), Sent(id+sd)), OutIter(jd), count_equal());
	CHECK(base(r.first) == id + sd);
	CHECK(base(r.second) == jd + 2);
	CHECK(jd[0] == 0);
	CHECK(jd[1] == 1);
	...
	int je[se] = {-1};
	count_equal::count = 0;
	r = stl2::unique_copy(stl2::ext::subrange(InIter(ie), Sent(ie+se)), OutIter(je), count_equal());
	CHECK(base(r.first) == ie + se);
	CHECK(base(r.second) == je + 3);
	CHECK(je[0] == 0);
	CHECK(je[1] == 1);
	...
	int jg[sg] = {-1};
	count_equal::count = 0;
	r = stl2::unique_copy(stl2::ext::subrange(InIter(ig), Sent(ig+sg)), OutIter(jg), count_equal());
	CHECK(base(r.first) == ig + sg);
	CHECK(base(r.second) == jg + 2);
	CHECK(jg[0] == 0);
	CHECK(jg[1] == 1);
	...
	int jh[sh] = {-1};
	count_equal::count = 0;
	r = stl2::unique_copy(stl2::ext::subrange(InIter(ih), Sent(ih+sh)), OutIter(jh), count_equal());
	CHECK(base(r.first) == ih + sh);
	CHECK(base(r.second) == jh + 2);
	CHECK(jh[0] == 0);
	CHECK(jh[1] == 1);
	...
	int ji[si] = {-1};
	count_equal::count = 0;
	r = stl2::unique_copy(stl2::ext::subrange(InIter(ii), Sent(ii+si)), OutIter(ji), count_equal());
	CHECK(base(r.first) == ii + si);
	CHECK(base(r.second) == ji + 3);
	CHECK(ji[0] == 0);
	CHECK(ji[1] == 1);

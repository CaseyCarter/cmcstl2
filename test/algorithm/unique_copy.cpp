...
test_iter()
{
	...
	int ja[sa] = {-1};
	count_equal::count = 0;
	auto r = stl2::unique_copy(InIter(ia), Sent(ia+sa), OutIter(ja), count_equal());
...
test_range()
{
	...
	int ja[sa] = {-1};
	count_equal::count = 0;
	auto r = stl2::unique_copy(::as_lvalue(stl2::subrange(InIter(ia), Sent(ia+sa))), OutIter(ja), count_equal());
	CHECK(base(r.in) == ia + sa);
	CHECK(base(r.out) == ja + sa);
	CHECK(ja[0] == 0);
	CHECK(count_equal::count == sa-1);
...
int main()
{
	...
	// Test projections:
	{
		...
		S ib[stl2::size(ia)];
		auto r = stl2::unique_copy(ia, ib, stl2::equal_to<>(), &S::i);

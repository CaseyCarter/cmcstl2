...
void test_iter()
{
	...
	int ib[sa] = {0};
	auto r = stl2::replace_copy(InIter(ia), Sent(ia+sa), OutIter(ib), 2, 5);
...
void test_rng()
{
	...
	auto r = stl2::replace_copy(rng, OutIter(ib), 2, 5);
	CHECK(base(r.in) == ia + sa);
	CHECK(base(r.out) == ib + sa);
	CHECK(ib[0] == 0);
	CHECK(ib[1] == 1);
...
int main()
{
	...
	// Test projection
	{
		...
		P out[stl2::size(in)] = {};
		auto r = stl2::replace_copy(in, out, 2, P{5, "5"}, &std::pair<int, std::string>::first);
		CHECK(r.in == stl2::end(in));
		CHECK(r.out == stl2::end(out));
		CHECK(out[0] == P{0, "0"});
		CHECK(out[1] == P{1, "1"});
		CHECK(out[2] == P{5, "5"});
		CHECK(out[3] == P{3, "3"});

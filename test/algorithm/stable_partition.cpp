...
test_range()
{
	...
	{  // check mixed
		P ap[] = { {0, 1}, {0, 2}, {1, 1}, {1, 2}, {2, 1}, {2, 2}, {3, 1}, {3, 2}, {4, 1}, {4, 2} };
		std::size_t size = ranges::size(ap);
		Iter r = ranges::stable_partition(ranges::ext::subrange(Iter(ap), Sent(ap+size)), odd_first());
		CHECK(base(r) == ap + 4);
		CHECK(ap[0] == P{1, 1});
		CHECK(ap[1] == P{1, 2});
		CHECK(ap[2] == P{3, 1});
	...
	}
	{
		...
		CHECK(ap[9] == P{4, 2});
		// check empty
		r = ranges::stable_partition(ranges::ext::subrange(Iter(ap), Sent(ap)), odd_first());
		CHECK(base(r) == ap);
		// check one true
		r = ranges::stable_partition(ranges::ext::subrange(Iter(ap), Sent(ap+1)), odd_first());
		CHECK(base(r) == ap+1);
		CHECK(ap[0] == P{1, 1});
		// check one false
		r = ranges::stable_partition(ranges::ext::subrange(Iter(ap+4), Sent(ap+5)), odd_first());
		CHECK(base(r) == ap+4);
		CHECK(ap[4] == P{0, 1});
	}
	{  // check all false

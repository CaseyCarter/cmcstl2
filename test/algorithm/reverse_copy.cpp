...
void test()
{
	// iterators
	{
		...
		int ja[sa] = {-1};
		auto p0 = stl2::reverse_copy(Iter(ia), Sent(ia), OutIter(ja));
		CHECK_EQUAL(ja, {-1});
		CHECK(p0.in == Iter(ia));
		CHECK(base(p0.out) == ja);
		auto p1 = stl2::reverse_copy(Iter(ia), Sent(ia+sa), OutIter(ja));
		CHECK_EQUAL(ja, {0});
		...
		int jb[sb] = {-1};
		auto p2 = stl2::reverse_copy(Iter(ib), Sent(ib+sb), OutIter(jb));
		CHECK_EQUAL(jb, {1, 0});
		...
		int jc[sc] = {-1};
		auto p3 = stl2::reverse_copy(Iter(ic), Sent(ic+sc), OutIter(jc));
		...
		int jd[sd] = {-1};
		auto p4 = stl2::reverse_copy(Iter(id), Sent(id+sd), OutIter(jd));
	...
	// ranges
	{
		...
		const unsigned sa = sizeof(ia)/sizeof(ia[0]);
		int ja[sa] = {-1};
		auto p0 = stl2::reverse_copy(::as_lvalue(stl2::subrange(Iter(ia), Sent(ia))), OutIter(ja));
		CHECK_EQUAL(ja, {-1});
		CHECK(p0.in == Iter(ia));
		CHECK(base(p0.out) == ja);
		auto p1 = stl2::reverse_copy(::as_lvalue(stl2::subrange(Iter(ia), Sent(ia+sa))), OutIter(ja));
		CHECK_EQUAL(ja, {0});
		CHECK(p1.in == Iter(ia+sa));
		CHECK(base(p1.out) == ja+sa);
		...
		const unsigned sb = sizeof(ib)/sizeof(ib[0]);
		int jb[sb] = {-1};
		auto p2 = stl2::reverse_copy(::as_lvalue(stl2::subrange(Iter(ib), Sent(ib+sb))), OutIter(jb));
		CHECK_EQUAL(jb, {1, 0});
		CHECK(p2.in == Iter(ib+sb));
		CHECK(base(p2.out) == jb+sb);
		...
		const unsigned sc = sizeof(ic)/sizeof(ic[0]);
		int jc[sc] = {-1};
		auto p3 = stl2::reverse_copy(::as_lvalue(stl2::subrange(Iter(ic), Sent(ic+sc))), OutIter(jc));
		CHECK_EQUAL(jc, {2, 1, 0});
		CHECK(p3.in == Iter(ic+sc));
		CHECK(base(p3.out) == jc+sc);
		...
		const unsigned sd = sizeof(id)/sizeof(id[0]);
		int jd[sd] = {-1};
		auto p4 = stl2::reverse_copy(::as_lvalue(stl2::subrange(Iter(id), Sent(id+sd))), OutIter(jd));
		CHECK_EQUAL(jd, {3, 2, 1, 0});
		CHECK(p4.in == Iter(id+sd));
		CHECK(base(p4.out) == jd+sd);

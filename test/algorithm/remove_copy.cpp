...
test_iter()
{
	...
	int ib[sa];
	auto r = stl2::remove_copy(InIter(ia), Sent(ia+sa), OutIter(ib), 2);
...
test_range()
{
	...
	constexpr unsigned sa = stl2::size(ia);
	int ib[sa];
	auto r = stl2::remove_copy(::as_lvalue(stl2::subrange(InIter(ia), Sent(ia+sa))), OutIter(ib), 2);
	CHECK(base(r.in) == ia + sa);
	CHECK(base(r.out) == ib + sa-3);
	CHECK(ib[0] == 0);
	CHECK(ib[1] == 1);
...
int main()
{
	...
	// Check projection
	{
		...
		constexpr unsigned sa = stl2::size(ia);
		S ib[sa];
		auto r = stl2::remove_copy(ia, ib, 2, &S::i);
		CHECK(r.in == ia + sa);
		CHECK(r.out == ib + sa-3);
		CHECK(ib[0].i == 0);
		CHECK(ib[1].i == 1);

...
void test()
{
    ...
    // ranges
    {
        int ia[] = {0};
        const unsigned sa = sizeof(ia)/sizeof(ia[0]);
        Iter i0 = stl2::reverse(::as_lvalue(stl2::ext::make_range(Iter(ia), Sent(ia))));
        ::check_equal(ia, {0});
        CHECK(i0 == Iter(ia));
        Iter i1 = stl2::reverse(::as_lvalue(stl2::ext::make_range(Iter(ia), Sent(ia+sa))));
        ::check_equal(ia, {0});
        CHECK(i1 == Iter(ia+sa));

        int ib[] = {0, 1};
        const unsigned sb = sizeof(ib)/sizeof(ib[0]);
        Iter i2 = stl2::reverse(::as_lvalue(stl2::ext::make_range(Iter(ib), Sent(ib+sb))));
        ::check_equal(ib, {1, 0});
        CHECK(i2 == Iter(ib+sb));

        int ic[] = {0, 1, 2};
        const unsigned sc = sizeof(ic)/sizeof(ic[0]);
        Iter i3 = stl2::reverse(::as_lvalue(stl2::ext::make_range(Iter(ic), Sent(ic+sc))));
        ::check_equal(ic, {2, 1, 0});
        CHECK(i3 == Iter(ic+sc));

        int id[] = {0, 1, 2, 3};
        const unsigned sd = sizeof(id)/sizeof(id[0]);
        Iter i4 = stl2::reverse(::as_lvalue(stl2::ext::make_range(Iter(id), Sent(id+sd))));
        ::check_equal(id, {3, 2, 1, 0});
        CHECK(i4 == Iter(id+sd));

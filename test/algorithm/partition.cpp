...
test_range()
{
	...
	int ia[] = {1, 2, 3, 4, 5, 6, 7, 8 ,9};
	const unsigned sa = sizeof(ia)/sizeof(ia[0]);
	Iter r = stl2::partition(stl2::ext::subrange(Iter(ia), Sent(ia + sa)), is_odd());
	CHECK(base(r) == ia + 5);
	for (int* i = ia; i < base(r); ++i)
		CHECK(is_odd()(*i));
	for (int* i = base(r); i < ia+sa; ++i)
		CHECK(!is_odd()(*i));
	// check empty
	r = stl2::partition(stl2::ext::subrange(Iter(ia), Sent(ia)), is_odd());
	CHECK(base(r) == ia);
	// check all false
	for (unsigned i = 0; i < sa; ++i)
		ia[i] = 2*i;
	r = stl2::partition(stl2::ext::subrange(Iter(ia), Sent(ia+sa)), is_odd());
	CHECK(base(r) == ia);
	// check all true
	for (unsigned i = 0; i < sa; ++i)
		ia[i] = 2*i+1;

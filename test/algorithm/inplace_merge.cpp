...
test_one_rng(unsigned N, unsigned M)
{
	...
	std::sort(ia, ia+M);
	std::sort(ia+M, ia+N);
	auto res = stl2::inplace_merge(stl2::ext::subrange(Iter(ia), Sent(ia+N)), Iter(ia+M));
	CHECK(res == Iter(ia+N));
	if(N > 0)
	{
		CHECK(ia[0] == 0);
	...
	std::sort(ia, ia+M);
	std::sort(ia+M, ia+N);
	auto res2 = stl2::inplace_merge(unref_view{stl2::ext::subrange(Iter(ia), Sent(ia+N))}, Iter(ia+M));

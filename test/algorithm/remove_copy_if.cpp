...
test_iter()
{
	...
	int ib[sa];
	auto r = stl2::remove_copy_if(InIter(ia), Sent(ia+sa), OutIter(ib), [](int i){return i == 2;});
...
test_range()
{
	...
	constexpr unsigned sa = stl2::size(ia);
	int ib[sa];
	auto r = stl2::remove_copy_if(::as_lvalue(stl2::subrange(InIter(ia), Sent(ia+sa))), OutIter(ib), [](int i){return i == 2;});
	CHECK(base(r.in) == ia + sa);
	CHECK(base(r.out) == ib + sa-3);
	CHECK(ib[0] == 0);
	CHECK(ib[1] == 1);
...
int main()
{
	...
	// Check projection
	{
		...
		constexpr unsigned sa = stl2::size(ia);
		S ib[sa];
		auto r = stl2::remove_copy_if(ia, ib, [](int i){return i == 2;}, &S::i);
		CHECK(r.in == ia + sa);
		CHECK(r.out == ib + sa-3);
		CHECK(ib[0].i == 0);
		CHECK(ib[1].i == 1);

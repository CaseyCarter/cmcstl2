...
int main()
{
    ...
    CHECK(stl2::ext::upper_bound(begin(a), size(a), a[0]) == &a[1]);
    CHECK(stl2::ext::upper_bound(begin(a), size(a), a[1], less<>()) == &a[2]);
    CHECK(stl2::ext::upper_bound(begin(a), size(a), 1, less<>(), &std::pair<int, int>::first) == &a[4]);

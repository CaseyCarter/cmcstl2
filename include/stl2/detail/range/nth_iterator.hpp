		...
		struct __nth_iterator {
			...
			operator()(R&& r, iter_difference_t<iterator_t<R>> n) const {
				...
				if constexpr (SizedRange<R>) {
					...
					constexpr bool CommonNonRandom =
						CommonRange<R> && !RandomAccessRange<R>;
					if (n >= size) {
						if constexpr (CommonNonRandom && !BidirectionalRange<R>) {
							// If the range is RandomAccess or Bidirectional, this
							// is just extra codegen with no performance
							// improvement over the following cases.
							return end(r);
						}
						n = size;
					}
					if constexpr (CommonNonRandom && BidirectionalRange<R>) {
						// Again, this would not be an improvement for
						// RandomAccess ranges.
						if (n > size / 2) {

...
STL2_OPEN_NAMESPACE {
  ...
  I2 partial_sort_copy(I1 first, S1 last, I2 result_first, S2 result_last,
    ...
    if(r != result_last) {
      ...
      __stl2::make_heap(result_first, r, __stl2::ref(comp), __stl2::ref(proj));
      const auto len = DifferenceType<I2>(r - result_first);
      for(; first != last; ++first) {
        auto&& x = *first;
        if(comp(proj(x), proj(*result_first))) {
          *result_first = (decltype(x) &&) x;
          detail::sift_down_n(result_first, len, result_first, __stl2::ref(comp), __stl2::ref(proj));
        }
      }
      __stl2::sort_heap(result_first, r, __stl2::ref(comp), __stl2::ref(proj));
    }
    return r;
  }
  ...
  partial_sort_copy(Rng1&& rng, Rng2&& result_rng,

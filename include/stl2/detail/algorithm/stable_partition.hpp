...
STL2_OPEN_NAMESPACE {
  namespace detail {
    namespace stable_part {
      Readable{I}
      using buf_t = detail::temporary_buffer<ValueType<I>>;

      template <ForwardIterator I, class Proj,
                IndirectCallablePredicate<Projected<I, Proj>> Pred>
      void skip_true(I& first, DifferenceType<I>& n, Pred& pred, Proj& proj) {
      ...
      template <ForwardIterator I, class Proj,
        ...
        requires Permutable<I>()
      tagged_pair<tag::begin(I), tag::end(I)>
      forward_reduce(I first, DifferenceType<I> n,
                     buf_t<I>& buf, Pred& pred, Proj& proj);
      ...
      template <BidirectionalIterator I, class Proj,
                IndirectCallablePredicate<Projected<I, Proj>> Pred>
        requires Permutable<I>()
      I bidirectional_reduce_front(I first, I last, DifferenceType<I> n,
                                   buf_t<I>& buf, Pred& pred, Proj& proj);

      template <BidirectionalIterator I, class Proj,
                IndirectCallablePredicate<Projected<I, Proj>> Pred>
        requires Permutable<I>()
      I bidirectional_reduce_back(I first, I last, DifferenceType<I> n,
                                  buf_t<I>& buf, Pred& pred, Proj& proj);

      template <BidirectionalIterator I, class Proj,
                IndirectCallablePredicate<Projected<I, Proj>> Pred>
        requires Permutable<I>()
      I bidirectional(I first, I last, DifferenceType<I> n,
                      buf_t<I>& buf, Pred& pred, Proj& proj) {
        // Precondition: !pred(proj(*first))
        // Precondition: pred(proj(*last))
        // Precondition: n == distance(first, last)
        STL2_ASSUME(n >= DifferenceType<I>(1));
      ...
      I bidirectional_reduce_front(I first, I last, DifferenceType<I> n,
        ...
        // Precondition: pred(proj(*last))
        // Precondition: n == distance(first, last)
        STL2_ASSUME(n >= DifferenceType<I>(0));
  ...
  namespace ext {
    ...
    I stable_partition_n(I first, DifferenceType<I> n,
      ...
      static constexpr DifferenceType<I> alloc_threshold = 3;
    ...
    template <BidirectionalIterator I, class Proj = identity,
              IndirectCallablePredicate<Projected<I, Proj>> Pred>
      requires Permutable<I>()
    I stable_partition_n(I first, I last, DifferenceType<I> n,
      ...
      // might want to make this a function of trivial assignment
      static constexpr DifferenceType<I> alloc_threshold = 4;
      using buf_t = detail::stable_part::buf_t<I>;

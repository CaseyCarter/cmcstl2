...
STL2_OPEN_NAMESPACE {
	...
	public:
		// FIXME(cmcstl2/issues/189) Non-conforming, because goto cannot be used in a constexpr context.
		//
		template<RandomAccessIterator I, Sentinel<I> S, class Comp = ranges::less<>, class Proj = identity>
		requires Sortable<I, Comp, Proj>
		/* constexpr */ I operator()(I first, I nth, S last, Comp comp = Comp{}, Proj proj = Proj{}) const
		{
			...
			restart:
				...
				case 3:
					{
						I m = first;
						__nth_element_fn::sort3(first, ++m, --end, comp, proj);
						return end_orig;
					}
				}
				if (len <= limit) {
					__nth_element_fn::selection_sort(first, end, comp, proj);
					return end_orig;
				}
				...
				I m = first + len/2;
				I lm1 = end;
				unsigned n_swaps = __nth_element_fn::sort3(first, m, --lm1, comp, proj);
				// *m is median
		...
		operator()(R&& r, iterator_t<R> nth, Comp comp = Comp{}, Proj proj = Proj{}) const
		{
			return (*this)(__stl2::begin(r), std::move(nth), __stl2::end(r),

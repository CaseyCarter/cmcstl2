...
STL2_OPEN_NAMESPACE {
	class __is_permutation_fn {
		...
		static constexpr bool __has_length(const I first, const S last, const D n) {
			...
			if constexpr (SizedSentinel<S, I>) {
				...
				if (__is_permutation_fn::__can_represent<iter_difference_t<I>>(n)) {
		...
		static constexpr bool __has_length(Rng&& rng, const D n) {
			...
			if constexpr (SizedRange<Rng>) {
				...
				return __is_permutation_fn::__has_length(__stl2::begin(rng), __stl2::end(rng), n);
			}
		}
		...
		__common_range_length(I1 first1, S1 last1, I2 first2, S2 last2) {
			...
			if constexpr (SizedSentinel<S1, I1>) {
				...
				return {__is_permutation_fn::__has_length(first2, last2, count), count};
			} else if constexpr (SizedSentinel<S2, I2>) {
				const auto count = last2 - first2;
				return {__is_permutation_fn::__has_length(first1, last1, count), static_cast<D>(count)};
			} else {
		...
		__common_range_length(Rng1&& rng1, Rng2&& rng2) {
			...
			if constexpr (SizedRange<Rng1>) {
				...
				return {__is_permutation_fn::__has_length(rng2, count), count};
			} else if constexpr (SizedRange<Rng2>) {
				auto count = __stl2::distance(rng2);
				return {__is_permutation_fn::__has_length(rng1, count), static_cast<D>(count)};
			} else {
				return __is_permutation_fn::__common_range_length(
			...
			Pred& pred, Proj1& proj1, Proj2& proj2)
		{
			...
			return __is_permutation_fn::__is_permutation_tail(counted.base(), std::move(mid2),
		...
		static bool impl(I1 first1, S1 last1, I2 first2, S2 last2, Pred pred = Pred{},
			Proj1 proj1 = Proj1{}, Proj2 proj2 = Proj2{})
		{
			if constexpr (SizedSentinel<S1, I1> || SizedSentinel<S2, I2>) {
				...
				if constexpr (SizedSentinel<S1, I1>) {
					count1 = last1 - first1;
					if (!__is_permutation_fn::__has_length(first2, last2, count1)) return false;
				} else {
					const auto count2 = last2 - first2;
					if (!__is_permutation_fn::__has_length(first1, last1, count2)) return false;
					count1 = static_cast<decltype(count1)>(count2);
				}
				return __is_permutation_fn::__is_permutation_trim(std::move(first1), std::move(first2),
				...
				auto [same_length, count] =
					__is_permutation_fn::__common_range_length(mid1, std::move(last1), mid2, std::move(last2));
				if (!same_length) return false;

				return __is_permutation_fn::__is_permutation_tail(std::move(mid1), std::move(mid2),
		...
		static bool impl(Rng1&& rng1, Rng2&& rng2, Pred pred = Pred{},
			Proj1 proj1 = Proj1{}, Proj2 proj2 = Proj2{})
		{
			if constexpr (SizedRange<Rng1> || SizedRange<Rng2>) {
				...
				if constexpr (SizedRange<Rng1>) {
					...
					if (!__is_permutation_fn::__has_length(rng2, count1)) return false;
				} else {
					const auto count2 = __stl2::distance(rng2);
					if (!__is_permutation_fn::__has_length(rng1, count2)) return false;
					count1 = static_cast<D>(count2);
				}
				return __is_permutation_fn::__is_permutation_trim(__stl2::begin(rng1),
					__stl2::begin(rng2), count1, pred, proj1, proj2);
			} else {
				return __is_permutation_fn::impl(
					__stl2::begin(rng1), __stl2::end(rng1),
					__stl2::begin(rng2), __stl2::end(rng2),
					std::ref(pred), std::ref(proj1), std::ref(proj2));
			}
	...
	public:
		...
		constexpr bool operator()(Rng1&& rng1, Rng2&& rng2, Pred pred = Pred{}, Proj1 proj1 = Proj1{},
			Proj2 proj2 = Proj2{}) const
		{
			return __is_permutation_fn::impl(
				static_cast<Rng1&&>(rng1), static_cast<Rng2&&>(rng2),
				std::ref(pred), std::ref(proj1), std::ref(proj2));
		}
	};

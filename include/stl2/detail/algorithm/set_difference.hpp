...
STL2_OPEN_NAMESPACE {
	...
	struct set_difference_result : __in_out_result<I, O> {};
	...
	struct __set_difference_fn {
			...
			Comp comp = Comp{}, Proj1 proj1 = Proj1{}, Proj2 proj2 = Proj2{}) const
		{
			...
			auto copy_result = __stl2::copy(std::move(first1), std::move(last1), std::move(result));
			return {std::move(copy_result.in), std::move(copy_result.out)};
		}
			...
			Comp comp = Comp{}, Proj1 proj1 = Proj1{}, Proj2 proj2 = Proj2{}) const
		{
			return (*this)(
				...
				__stl2::begin(r2), __stl2::end(r2), std::move(result),
